...
```
После разрешения конфликта слияния обязательно выполнить `commit` о слиянии и отсутствии конфликтов.
...
# 3. Авторизовать "подружить" локальный и удаленный репозиторий. 

GitHub при создании нового репозитория даст подсказки как это можно сделать.

# 4. Отправить коммандой `push` локальный репозиторий в удаленный (на GitHub).

При этом вам, возможно, прийдется авторизоваться в удаленном репозитории.

# 5. Произвести изменения "с другого компьютера"

# 6. Выкачать коммандой `pull` актуальное сосотояние из удаленного репозитория.

# 7. Команда `git clone` клонирует репозиторий с GitHub.

# 8. Работа с Open Source

+ сделать `fork` интересующего репозитория.
+ делаем `git clone` для нашей версии репозитория.
+ создаем ветку `git branch` с предлогаемыми изменениями.
+ производим все изменения только в этой ветке.
+ отправляем изменения в свой аккаунт `push`.
+ в окне на GitHub появится возможность отправить `pull request` в исходный репозиторий.
...
Наприммер, **вот так** или __вот так__.

Альтернативные способы выделения курсивом и полужирным нужны что бы совмещать оба способа. 

Например, _курсивом и **полужирным курсивом**_
...
Чтобы добавить нумерованные списки необходимо их пронумеровать 1,2,3... 
...
## Ссылки
 ...
 Пример:
 + <http://google.com/>
 + <no_comment@gmail.com>

Если вставлять с анкором, то тогда текст ссылки заключается в [ ] скобки, а адрес страницы – в ( ) скобки. Рядом с URL можно прописать тайтл, его объявляют в кавычках он тоже остается внутри круглых скобок.

Пример:
+ Это ссылка с анкором, тайтлом и заголовком - это поиск [Google](http://google.com/ "Поиск")
+ Это ссылка без заголовка и тайтла - [Поиск](http://google.com/)

## Цитаты

 Для цитаты нужна только однаугловая скобка (>). 
...
Мы разобрали с десяток различных символов, которые используются в разметке Markdown. Но бывает что эти символы нужны нам в самом тексте. Чтобы спецсимволы не исчезали и не влияли на оформление, нужно использовать экранирование. Этим целям служит обратная косая черта ( \ ). 
Пример:
